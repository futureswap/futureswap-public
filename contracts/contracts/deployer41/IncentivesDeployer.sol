...
contract IncentivesDeployer is FsBase {
    ...
    function deployOpenInterestIncentives(address incentivesHook, address rewardsToken)
        public
        returns (address)
    {
    ...
    function deployTradingFeeIncentives(address incentivesHook, address rewardsToken)
        public
        returns (address)
    {
        // Slither infers type for `initialize.selector` to be `uint256`, while the first argument
    ...
    function deployIncentivesHook(address exchange, address _rewardsToken)
        external
        onlyOwner
        returns (
        ...
        )
    {
        ...
        address incentivesHook = address(new IncentivesHook(exchange));
        address openInterestIncentives = deployOpenInterestIncentives(incentivesHook, rewardsToken);
        address tradingFeeIncentives = deployTradingFeeIncentives(incentivesHook, rewardsToken);
        ...
        // Return deployed addresses here so staticcalls can predict what the addresses would be.
        return (incentivesHook, openInterestIncentives, tradingFeeIncentives);
    }
