...
contract ExchangeDeployer is FsBase {
    ...
    /// @param _assetToken The address of the token that will be used as the asset in the exchange.
    /// @param _stableToken The address of the token that will be used as the stable in the exchange.
    /// @param _liquidityTokenName Name of the liquidity token associated with the exchange.
    /// @param _liquidityTokenSymbol Symbol of the liquidity token associated with the exchange.
    /// @param _priceOracle The oracle used by the exchange to get stable or asset prices.
    /// @param _ammAdapter The AMM adapter used by the exchange's spot market amm.
    /// @param _exchangeConfig The first part of the exchange's config.
    /// @param ammConfig The AMM's config.
    /// @return Addresses for deployed contracts.
    function createExchangeWithSpotMarketAmm(
        address _assetToken,
        address _stableToken,
        string calldata _liquidityTokenName,
        string calldata _liquidityTokenSymbol,
        address _priceOracle,
        address _ammAdapter,
        ExchangeLedger.ExchangeConfig calldata _exchangeConfig,
        ...
        data.assetToken = _assetToken;
        data.stableToken = _stableToken;
        data.priceOracle = _priceOracle;
        data.ammAdapter = _ammAdapter;
        data.tokenVault = address(new TokenVault(admin));
        data.liquidityToken = address(
            new LiquidityToken(_liquidityTokenName, _liquidityTokenSymbol)
        );

        deployExchangeLedger(data, _exchangeConfig);
        data.tradeRouter = DeployerLibrary.deployTradeRouter(data, wethToken);
        deploySpotMarketAmm(data, ammConfig);
    ...
    function deployExchangeLedger(
        ...
        ExchangeLedger(exchangeLedger).setExchangeConfig(_exchangeConfig);
